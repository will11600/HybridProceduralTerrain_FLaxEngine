...
internal unsafe sealed class FatPointer<T>(int elementCount) : IDisposable where T : unmanaged
{
    private bool _disposed;

    public int Length { get; } = elementCount;

    public T* Buffer { get; private set; } = (T*)NativeMemory.Alloc((nuint)elementCount, (nuint)sizeof(T));
